...
The architecture cleanup and no-duplication checklist is
`docs/no-duplication-architecture-audit.md`. It records the current canonical
production paths, dev/test-only paths, near-duplicate boundaries, and the
required cleanup steps so strict signing, DKG, IT-VSS, Power2Round, and nonce
preprocessing do not grow parallel implementations.
...
- [ ] No release path publishes or depends on exact `A*secret` images (`A*s1_i`, `A*nonce` coefficients, or `Phi = A*secret`).
  Current status: public DKG `A*s1_i` artifacts were removed from the normal
  DKG API: `As1Commitment`, `DkgCommitPayload.as1_commitment`,
  `DkgPublicOutput.as1_commitments`, and `CommitmentSet::As1` are absent from
  `talus-dkg` production structs and transcript code. Public
  preprocessing `A*nonce` artifacts are also test/dev-only:
  `DistributedNonceShare.ay_commitment`, `MaskedBroadcastClearAudit`,
  `ClearMaskedBroadcastConsistencyVerifier`, and `CutAndChooseAuditPlan` are
  absent from normal builds. Source scanner tests now assert these symbols stay
  out of normal production exports or are confined to explicit dev/test
  modules.
- [x] `CommitmentBackedPartialVerifier` and any public-linear-image blame path are gated to tests or explicit insecure paper-compatibility builds.
- [ ] Strict production signing exposes no rejected `z_i`, aggregate `z`, hints, validity bits, or failure reasons.
  Current status: `talus-mpc` now exposes the strict production signing
  boundary: `StrictSignRequest`, `BccCertifiedTokenBatch`,
  `ConsumedBccCertifiedTokenBatch`, `StrictPrivateSigningBackend`, and
  `sign_strict_no_rejected_z`. The wrapper consumes every certified token in
  the batch durably before the private backend receives token material, and it
  runs final verification before returning output. `StrictSigningSession` now
  provides the production-facing app-driver facade with `start`,
  `handle_private`, `handle_broadcast`, `next_outbound`, `finish`, and
  `into_parts`; it accepts only the production `StrictSignMpc` wire domain,
  rejects legacy partial-signature traffic and malformed session/context
  bindings, queues outbound strict MPC private/broadcast messages, and makes
  success/failure terminal after finish. `StrictSigningSessionStore` and
  `StrictSigningSessionCursor` now
  persist a deterministic strict signing session id, request hash, fixed token
  ids, coarse phase (`Started`, `TokensConsumed`, `Finished`, `Failed`),
  optional runtime slot, selected signature hash after success, accepted strict
  MPC message hashes, outbound strict MPC message hashes, and the strict MPC
  wire transcript hash.
  `FileStrictSigningSessionStore` survives reopen and blocks failed/finished
  sessions from being restarted with the same consumed token batch.
  `StrictSigningRuntimeSlot` names the future distributed vector IT-MPC phase
  slots: response preparation, response-bound checks, hint checks, private
  selection, and selected opening. `StrictSigningDistributedRuntime` is now the
  slot-driven app-message boundary for distributed strict MPC payloads:
  `StrictSigningSession` validates `StrictSignMpcPayload`, delegates decoded
  payloads to the runtime, records accepted/outbound message hashes, persists
  per-slot phase/sender/outbound/transcript progress, requires every signer to
  contribute before a slot may complete, persists completed runtime slots in
  order, and rejects duplicates, wrong phases, incomplete completion, and
  replays after restart.
  `StrictSigningRuntime` remains the local private-computation boundary behind
  strict signing, and
  `StrictSigningRuntimeObserver` persists every slot transition through the
  session cursor store before the corresponding phase executes.
  `talus-wire` now exposes `StrictSignMpcPayload` with opaque backend payload
  bytes and typed slots for those same phases; it does not expose partial
  response fields, per-candidate pass bits, or failure reasons.
  `StrictSigningPhaseDriver`
  now enforces the production order from durable consumption through final
  verification. `StrictPrivateSigningBackend` now returns
  `StrictSelectedSignature` plus `StrictSigningEvidence`, so backend outputs
  are constrained to one selected signature, coarse response-check counters,
  selected public priority, selected signature hash, and backend transcript
  hash. The strict wrapper rejects response-check evidence whose counters do
  not match the consumed batch shape. `StrictCandidateMetadata` derives public
  per-token challenge/priority metadata without response shares, aggregate z,
  hints, private validity bits, or failure details. `StrictResponseCheckPhaseDriver`
  now enforces the inner private-circuit phase order: candidate metadata,
  shared responses, response-bound checks, hint checks, private pass bits,
  priority selection, and selected-only opening. `StrictResponseBoundCheckBackend`
  defines the production boundary for the first private predicate, returning
  only public shape evidence while keeping per-candidate predicate bits inside
  the backend. `StrictHintCheckBackend` defines the matching production
  boundary for private HighBits/hint and hint-weight checks, again returning
  only public shape evidence. `StrictPrivateSelectionBackend` defines the
  production boundary for private pass-bit combination and lowest-priority
  valid-candidate selection; it returns only the selected candidate and public
  selection evidence. `StrictSelectedOpeningBackend` defines the selected-only
  opening boundary; it receives exactly the selected candidate handle and
  returns only public opening evidence. `ProductionStrictSigningBackend` now
  composes the production trait stack through one normal entry point.
  `ProductionVectorResponsePreparationBackend` prepares vector responses from
  provided private `y`/`s1` polynomial shares. `ProductionVectorResponseBoundCheckBackend`
  evaluates the strict response bound over opaque vector handles.
  `ProductionVectorHintCheckBackend` computes `A*z - c*t1*2^d`, TALUS hints,
  and FIPS candidate encoding inside backend state. `ProductionVectorPrivateSelectionBackend`
  selects the lowest-priority candidate whose private checks passed, and
  `ProductionVectorSelectedOpeningBackend` opens only that selected candidate.
  The vector candidate handles are now consumed and returned by each phase, so
  the production-facing strict stack no longer depends on hidden `Rc<RefCell>`
  style local state to carry candidate internals across phase objects.
  The remaining production hardening work is the distributed transport-backed
  honest-majority IT-MPC runtime behind these vector operations.
  `LocalStrictPolynomialSigningBackend` provides a dev/test-only executable
  strict backend without clear partial transport. The production distributed
  private response-check circuit itself remains pending.
- [x] Current clear partial-signature adapters are gated as local/test or explicit research/paper-compatibility builds, not production.
...
## Milestone 5: TALUS-MPC Preprocessing And Token Certification

Implementation steps:

- [x] Implement preprocessing session facade.
  Current status: `talus-mpc/src/local.rs` contains
  `PreprocessingSessionOptions` and `PreprocessingSession`, with
  `start`, `handle_private`, `handle_broadcast`, `next_outbound`, and
  `finish -> CertifiedToken`. The current session handles broadcast commit/open
  messages and rejects private messages because nonce-generation private
  delivery is not yet app-driven.
- [x] Implement deterministic in-process party harness for tests and local
  regression.
- [x] Implement session IDs, transcript binding, and uniqueness checks.
- [x] Implement nonce share generation core.
  Current status: `generate_distributed_nonce_shares` creates transcript-bound
  nonce residues, certifies them, Shamir-shares aggregate nonce coefficients,
  and returns only each party's local `DistributedNonceShare` plus public
  evidence. `DistributedNonceShare` no longer exposes a public exact `A*y`
  commitment.
- [ ] Replace in-process nonce-generation orchestration with app-driven
  production IT-VSS/IT-MPC private-delivery phases.
...
- [x] Add masked-broadcast consistency verifier boundary.
  Current status: `MaskedBroadcastConsistencyVerifier` and
  `ProductMaskedBroadcastConsistencyVerifier` are normal preprocessing
  boundaries. The product verifier currently checks a deterministic
  statement/proof hash and recomputes the masked-broadcast envelope from local
  certified material. This is a production-shaped gate, not the final private
  distributed proof.
- [ ] Replace deterministic masked-broadcast consistency proof hashes with a
  final private certification backend or production IT-MPC evidence.
- [x] Implement vector CEF arithmetic and BCC token admission.
  Current status: `certify_vector_carry_compare_and_cef` computes CEF across
  all coefficients/signers, uses the approved `+ delta` correction, checks BCC,
  and emits typed CarryCompare/BCC evidence for token admission.
...
- [x] Implement token certification object.
  Current status: `CertifiedToken` carries `PreChallengeCertificationEvidence`
  and a derived policy. `TokenPool` rejects uncertified tokens and duplicate
  session ids.
- [ ] Add durable preprocessing token-pool persistence. Session-id persistence
  exists through `FileSessionRegistry`, but token inventory is still in-memory.
...
- [x] Product masked-broadcast consistency evidence is checked before token certification.
- [ ] Final masked-broadcast consistency evidence is produced by the private
  production backend, not deterministic local recomputation.
...
- [x] Gate `CommitmentBackedPartialVerifier` and public-linear-image blame as test/research only.
  Current status: the clear partial `z_i` shell, `CommitmentBackedPartialVerifier`,
  public `A*y_i`/`A*s1_i` online verifier artifacts, and the clear partial
  wire payload now compile only under `cfg(test)` or the explicit non-default
  `paper-fast-dev` feature. Normal callers must use the explicit
  `talus_mpc::dev_backends` / `talus_wire::dev_backends` paths to reach them.
  The workspace feature graph no longer enables `paper-fast-dev`
  transitively: `talus-tests` requires its explicit `paper-fast-dev` feature
  for paper-compatible attack/integration harnesses, while
  `cargo check --workspace --features production-release-checks` passes
  without compiling the insecure dev modules.
  Clear masked-broadcast audit helpers and cut-and-choose audit plans now live
  in `talus_mpc::local_dev`, re-exported only from `dev_backends`; the normal
  `DistributedNonceShare` no longer exposes `ay_commitment`. The DKG signing
  integration helper no longer constructs `CommitmentBackedPartialVerifier` or
  `A*s1` verifier commitments, and `talus-tests` includes a cross-crate
  production-source scan for forbidden paper-compatible artifacts.
...
- [ ] Implement reviewed PQ key-share provisioning mode.
  Current status: `talus-dkg` now exposes `ProvisionedKeyShare` and `import_provisioned_key_shares` for explicit reviewed setup packages. The importer requires exactly one package per configured party, identical transcript-bound public output, nonzero ceremony transcript hash, suite-correct public-key and `t1` lengths, complete pairwise seed commitment party sets, owner-bound secret packages, non-empty secret-share fields, and canonical typed `s1_share` bytes for the configured suite/party/interpolation point. It deliberately does not require or accept public `A*s1_i` commitments. The external reviewed provisioning ceremony and PQ channel/authentication implementation remain pending; this is not a silent trusted-dealer path.
...
- [ ] Finish pre-challenge masked-broadcast and CarryCompare certification.
  Current status: this is not a blank task. `talus-mpc` already has a
  production-facing `PreprocessingSession`, masked-broadcast commit/open,
  product consistency verifier boundary, typed certification evidence,
  `CertifiedToken`/`TokenPool` policy gates, session-id persistence, and vector
  CEF/BCC token admission with the approved `+ delta` formula. The remaining
  release blockers are narrower:
  ```text
  - app-driven nonce generation through production IT-VSS/IT-MPC;
  - private distributed masked-broadcast consistency evidence instead of the
    current deterministic statement/proof hash and local recomputation;
  - production vector IT-MPC evidence for CarryCompare/CEF/BCC instead of
    local aggregate witnesses;
  - durable preprocessing token-pool inventory and restart tests;
  - token-batch/chunk counters and performance gates.
  ```
  Post-challenge final verification failure must consume the token and reveal
  no honest nonce material by default. ZK, cut-and-choose, or
  reveal-on-failure forensic paths remain optional diagnostic test paths
